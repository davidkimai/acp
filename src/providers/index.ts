...
class TinyFishLiveProvider implements OpportunityProvider {
  ...
  async search(objective: Objective, sources: SourceDefinition[]): Promise<ProviderSearchResult[]> {
    if (config.tinyfishApiKey) {
      try {
        return await this.searchViaApi(objective, sources);
      } catch (error) {
        const cliReady = await this.assertTinyFishCliReady();
        if (!cliReady) {
          throw error;
        }
    ...
    const cliReady = await this.assertTinyFishCliReady();
    if (!cliReady) {
      throw new Error("TinyFish API key is not configured, and TinyFish CLI is not installed or not authenticated.");
    }
    return Promise.all(sources.map((source) => this.runSource(objective, source)));
  }
  ...
  private async assertTinyFishCliReady(): Promise<boolean> {
...
async function tinyfishRequest<T>(pathname: string, init: RequestInit): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const response = await fetch(`${config.tinyfishApiBaseUrl}${pathname}`, {
        ...init,
        headers: {
          ...init.headers,
      ...
      if (!response.ok) {
        ...
        if (response.status >= 500 && attempt < maxAttempts) {
          lastError = new Error(`TinyFish API request failed (${response.status}): ${message}`);
